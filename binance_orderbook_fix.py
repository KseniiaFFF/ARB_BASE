...
class BinanceOrderBook:
    ...
    common_orderbook: OrderBook | None = None
...
def build_common_orderbook(
    ...
    if orderbook.common_orderbook is None:
        ...
        orderbook.common_orderbook = OrderBook(
            exchange="binance",
            symbol=orderbook.symbol,
            bids=LazySortedLevels(
                orderbook.bids,
                reverse=True,
            ),
            asks=LazySortedLevels(
                orderbook.asks,
                reverse=False,
            ),
            timestamp=orderbook.last_event_time,
        ...
        orderbook.common_orderbook.timestamp = (
            orderbook.last_event_time
        )

        orderbook.common_orderbook.received_at = (
            orderbook.last_received_at
        )

        orderbook.common_orderbook.received_at_ns = (
            orderbook.last_received_at_ns
        )

        orderbook.common_orderbook.update_id = (
            orderbook.last_update_id
        )
                    ...
                    if not synced:
                        ...
                        if (
                            ...
                            if synchronize_from_buffer(
                                ...
                                # Snapshot не удалось состыковать с текущим буфером.
                                # Старый snapshot больше не используем: получаем новый.
                                snapshot_task = None
